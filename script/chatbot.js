...
module.exports.run = async ({ api, event, args }) => {
  try {
    ...
    if (query) {
      const processingMessage = await api.sendMessage(`Asking chatbot. Please wait a moment...`, event.threadID);

      const response = await axios.get(`https://api.freegpt4.ddns.net/?text=${encodeURIComponent(query)}`);

      if (response.data) {
        await api.sendMessage({ body: response.data.trim() }, event.threadID, event.messageID);
        console.log(`Sent PI's response to the user`);
      } else {
        throw new Error(`Invalid or missing response from chatbot API`);
      }
      
      await api.unsendMessage(processingMessage.messageID);
    }
  } catch (error) {
    console.error(`❌ | Failed to get chatbot's response: ${error.message}`);
